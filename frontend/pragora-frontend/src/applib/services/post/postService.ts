...
async function diagnoseSinglePostView(postId: number): Promise<any> {
  try {
    console.log(`Diagnostic: Fetching post ${postId}`);

    // Get the token
    const token = authService.getToken();
    console.log(`Diagnostic: Auth token available: ${!!token}`);

    // API URL
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
    console.log(`Diagnostic: Using API URL: ${apiUrl}`);

    // Try first with fetch API directly
    console.log(`Diagnostic: Trying with fetch API...`);
    const fetchResponse = await fetch(`${apiUrl}/posts/${postId}`, {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
        'Authorization': token ? `Bearer ${token}` : '',
    ...
    console.log(`Diagnostic: Fetch response status: ${fetchResponse.status}`);
    ...
    try {
      ...
      console.log(`Diagnostic: Raw response text: ${text.substring(0, 200)}...`);
      fetchData = JSON.parse(text);
    } catch (e) {
      fetchError = e;
      console.error(`Diagnostic: Error parsing fetch response: ${e}`);
    }

    // Now try with axios (used by PostFeed)
    console.log(`Diagnostic: Trying with axios...`);
    let axiosResponse = null;
    let axiosError = null;

    try {
      const axios = (await import('axios')).default;
      axiosResponse = await axios.get(`${apiUrl}/posts/${postId}`, {
        headers: {
          'Authorization': token ? `Bearer ${token}` : '',
          'Cache-Control': 'no-cache, no-store',
          'Pragma': 'no-cache'
      ...
      console.log(`Diagnostic: Axios response status: ${axiosResponse.status}`);
    } catch (e) {
      axiosError = e;
      console.error(`Diagnostic: Error with axios: ${e}`);
    }
    ...
    console.error(`Diagnostic: Top-level error: ${error}`);
    return { error };
  }
}

// Also export this function
export { diagnoseSinglePostView };
