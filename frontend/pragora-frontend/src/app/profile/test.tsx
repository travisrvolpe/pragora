...
export default function ProfilePage() {
  ...
  const [tabData, setTabData] = useState<ProfileTabData>({
    posts: { data: [], isLoading: false, error: null },
    comments: { data: [], isLoading: false, error: null },
    drafts: { data: [], isLoading: false, error: null },
    saved: { data: [], isLoading: false, error: null }
  });
  ...
  useEffect(() => {
      if (activeTab === 'posts' || activeTab === 'comments' || activeTab === 'saved') {
          fetchTabContent(activeTab as keyof ProfileTabData);
      }
      }, [activeTab]);

  const fetchTabContent = async (tab: keyof ProfileTabData) => {
  setTabData(prev => ({
    ...prev,
  ...
  try {
    const response = await fetch(`/api/profiles/me/${tab}`);
...
const renderComments = (comments: UserComment[]) => (
  <div className="space-y-4">
    {comments.map(comment => (
      <div key={comment.comment_id} className="bg-white p-4 rounded-lg shadow">
...
const renderTabContent = () => {
  switch (activeTab) {
    case 'overview':
      return /* your overview content */;
    case 'posts':
      return tabData.posts.isLoading ? (
        <div className="text-center py-8">Loading posts...</div>
      ) : tabData.posts.error ? (
        <div className="text-red-500 text-center py-8">{tabData.posts.error}</div>
      ) : (
        renderPosts(tabData.posts.data)
      );
    case 'comments':
      return /* your comments content */;
    case 'network':
      return <div className="text-center text-gray-500 py-8">Network section coming soon...</div>;
    case 'settings':
